package com.example.matematyczneworlde
...
class MainActivity : AppCompatActivity() {
            ...
            if (isEquationValid(equationString)) {
                ...
                if (leftSideValue == rightSideValue) {
                    ...
                    if (checkRowForWin(listOfTextViews[currentListIndex])) {
                        Toast.makeText(this@MainActivity, "Gratulacje, wygrałeś!", Toast.LENGTH_SHORT).show()
                        disableButtons()
                    } else if (currentListIndex == 5) {
                        Toast.makeText(this@MainActivity, "Niestety przegrałeś. Równanie, którego szukałeś to: $randomEquation", Toast.LENGTH_LONG).show()
                        disableButtons()
                    } else {
                        currentListIndex = (currentListIndex + 1) % listOfTextViews.size // Przejdź do następnego rzędu
                    }
                } else {
                    Toast.makeText(this@MainActivity, "Invalid equation: $equationString", Toast.LENGTH_SHORT).show()
                }
            } else {
                Toast.makeText(this@MainActivity, "Invalid equation: $equationString", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun compareEquations(userEquation: String, randomEquation: String, textViewList: List<TextView>) {
        ...
        for (i in 0 until maxEquationLength) {
            ...
            val colorResId = when {
                userChar == ' ' -> R.color.red
                userChar == randomChar -> R.color.green
                randomEquation.contains(userChar) -> R.color.yellow
                else -> R.color.red
            }
            ...
            if (resources.getColor(colorResId, null) != currentColor) {
                val rotateAnimation = AnimationUtils.loadAnimation(this@MainActivity, R.anim.rotate_animation)
