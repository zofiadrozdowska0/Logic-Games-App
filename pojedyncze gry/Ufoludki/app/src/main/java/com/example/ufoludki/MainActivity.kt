...
class MainActivity : AppCompatActivity() {
    ...
    private fun ustawParametryTrudnosci(poziom: String) {
        ...
        when (poziom) {
            "Łatwy" -> {
                ...
                czas_pirata_2 = 2000
                czas_animacji = 2000
                listaPiratowWchodzacych.addAll(listOf(R.drawable.zielony_pirat))
                listaPiratowWychodzacych.addAll(listOf(R.drawable.zielony_pirat))
            }
            "Średni" -> {
                czas_pirata_1 = 500
                czas_pirata_2 = 2000
                czas_animacji = 1500
                listaPiratowWchodzacych.addAll(listOf(R.drawable.zielony_pirat,R.drawable.zielony_pirat,R.drawable.zielony_pirat,R.drawable.czerwony_pirat, R.drawable.niebieski_pirat))
                listaPiratowWychodzacych.addAll(listOf(R.drawable.zielony_pirat, R.drawable.niebieski_pirat))
            }
            "Trudny" -> {
                czas_pirata_1 = 300
                czas_pirata_2 = 1000
                czas_animacji = 1000
                listaPiratowWchodzacych.addAll(listOf(R.drawable.zielony_pirat,R.drawable.zielony_pirat,R.drawable.zielony_pirat,R.drawable.czerwony_pirat, R.drawable.czerwony_pirat, R.drawable.niebieski_pirat))
                listaPiratowWychodzacych.addAll(listOf(R.drawable.zielony_pirat,R.drawable.zielony_pirat,R.drawable.niebieski_pirat))
            }
        }
    }
        ...
        val animacja = TranslateAnimation(
            ...
            setAnimationListener(object : Animation.AnimationListener {
                ...
                override fun onAnimationEnd(animation: Animation?) {
                    ...
                    when (obrazPirata) {
                        R.drawable.zielony_pirat -> wartoscLicznika += 1
                        R.drawable.czerwony_pirat -> wartoscLicznika *= 2
                        R.drawable.niebieski_pirat -> liczbaNiebieskichPiratow += 1 // Inkrementacja
                    }
    ...
    private fun stworzIAnimujPirataWychodzacego(obrazPirata: Int) {
        ...
        if (obrazPirata == R.drawable.zielony_pirat && wartoscLicznika <= 0) return

        // Warunek dla niebieskiego pirata - może wyjść tylko jeśli jakiś wszedł
        if (obrazPirata == R.drawable.niebieski_pirat && liczbaNiebieskichPiratow <= 0) return
        animacjaTrwa = true
        val pirat = ImageView(this).apply {
            setImageResource(obrazPirata)
            val wysokoscPirata = statek.height /5
        ...
        val animacja = TranslateAnimation(
                ...
                setAnimationListener(object : Animation.AnimationListener {
                    ...
                    override fun onAnimationEnd(animation: Animation?) {
                        layout.removeView(pirat) // Usunięcie widoku pirata po zakończeniu animacji
                        animacjaTrwa = false // Ustawienie flagi na false, aby umożliwić kolejne animacje
                        if (obrazPirata == R.drawable.zielony_pirat) {
                            wartoscLicznika -= 1
                        } else if (obrazPirata == R.drawable.niebieski_pirat) {
